    ...
    <div class="container">
        <div class="row">           
            <div class="col-md-12 title">
                <h1 class="text-left">Downloads</h1>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="list-group">
                    <?php
                        ...
                        $select = $conexao->prepare("SELECT * FROM tb_downloads LIMIT $inicioExibir, $exibir");
                        $select->execute();
                        while ($dados=$select->fetch(PDO::FETCH_ASSOC)) {
                    ?>
                    <a href="<?php echo $dados['link'] ?>" target="
                    _blank" class="list-group-item list-group-item-green"><i class="fas fa-download"></i> <?php echo $dados['nome'] ?></a>
                    <?php } ?>
                </div>
            </div>
        </div>
